...
# Adjusting the min and max values for visibility
...
def plot_clusters1(data_with_clusters, centroids, x_column, y_column):
    ...
    for cluster in data_with_clusters['Cluster'].unique():
        cluster_data = data_with_clusters[data_with_clusters['Cluster'] == cluster]
        plt.scatter(cluster_data[x_column], cluster_data[y_column], label=f'Cluster {cluster}', alpha=0.7, s=100)

    plt.scatter(centroids[:, data_with_clusters.columns.get_loc(x_column)],
                centroids[:, data_with_clusters.columns.get_loc(y_column)],
                marker='D', s=100, linewidths=2, color='black', label='Centroids')

    # Setting limits based on the range of data points and centroids
    x_min_value = min(data_with_clusters[x_column].min(), centroids[:, data_with_clusters.columns.get_loc(x_column)].min()) - 1
    x_max_value = max(data_with_clusters[x_column].max(), centroids[:, data_with_clusters.columns.get_loc(x_column)].max()) + 1
    y_min_value = min(data_with_clusters[y_column].min(), centroids[:, data_with_clusters.columns.get_loc(y_column)].min()) - 1
    y_max_value = max(data_with_clusters[y_column].max(), centroids[:, data_with_clusters.columns.get_loc(y_column)].max()) + 1
...
agricultural_data_with_clusters, agricultural_data_with_clusters_backscaled, agricultural_centroids = kmeans_clustering(agricultural_data, n_clusters=3, init_method='k-means++')
